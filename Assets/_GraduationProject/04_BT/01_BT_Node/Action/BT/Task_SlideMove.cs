...
public class Task_SlideMove : Node
{
    ...
    public override void OnEnter()
    {
        ...
        // // BTDebug.Log($"<color=cyan>[Task_SlideMove]</color> 슬라이드 시작. 기준점: {_centerPos}, 첫 목표: {(_movingRight ? "Right" : "Left")}");
    }
    ...
    protected override NodeState OnUpdate()
    {
        ...
        if (reached)
        {
            ...
            // // BTDebug.Log($"<color=cyan>[Task_SlideMove]</color> 목표 도달. 방향 전환 -> {(_movingRight ? "Right" : "Left")}, 실제 거리: {actualDist:F2}m");
        }
        else
        {
            ai.destination = _currentTarget;
