...
public class Task_AnimationNode : Node
{
    ...
    public override void OnEnter()
    {
        ...
        // // BTDebug.Log($"<color=white>[Task_AnimationNode]</color> '{triggerName}' 트리거 발송. 애니메이션 완료 및 {postDelayTime}초 대기 시작.");
    }
    ...
    protected override NodeState OnUpdate()
    {
        ...
        if (!_isAnimFinished)
        {
            if (Handler != null && Handler.IsActionFinished)
            {
                ...
                // // BTDebug.Log($"<color=white>[Task_AnimationNode]</color> 애니메이션 종료 신호 감지. 포스트 딜레이({postDelayTime}s) 대기 시작.");
            }
        ...
        {
            // // BTDebug.Log($"<color=white>[Task_AnimationNode]</color> 모든 대기 완료. SUCCESS 반환.");
